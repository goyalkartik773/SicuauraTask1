    ...
    <header
      className={cn(
        ...
        activeTransparent
          ...
          : 'bg-white text-brand-charcoal border-b border-gray-100 shadow-sm'
      )}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={cn('flex items-center justify-between transition-all duration-300', activeTransparent ? 'h-16' : 'h-16 md:h-16')}>
          {/* Left: Logo */}
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center tracking-tight select-none">
              ...
              <span
                className={cn(
                  'font-display font-light text-xl ml-0.5 transition-colors duration-300',
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      'text-sm font-body tracking-wide font-medium transition-colors hover:text-brand-rose',
                      isActive
                        ? 'text-brand-rose'
                        : activeTransparent
          ...
          <div className="hidden md:block flex-1 max-w-md mx-8">
            <form onSubmit={handleSearchSubmit} className="relative w-full">
              <input
                ...
                className={cn(
                  'w-full pl-10 pr-4 py-1.5 text-sm rounded-full border outline-none transition-all focus:ring-2 focus:ring-brand-rose focus:border-transparent',
                  activeTransparent
          ...
          <div className="px-4 pt-3 pb-6 space-y-4">
            ...
            <form onSubmit={handleSearchSubmit} className="relative w-full">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search sarees, lehengas, suits..."
                className="w-full pl-10 pr-4 py-2 text-sm bg-gray-50 border border-gray-200 rounded-full outline-none focus:ring-2 focus:ring-brand-rose focus:border-transparent"
              />
              <button
                type="submit"
                className="absolute left-3.5 top-1/2 -translate-y-1/2 flex items-center justify-center"
